    ...
    <Html center>
      <div className="flex flex-col items-center justify-center min-w-max">
        ...
        <div className="mt-4 flex flex-col items-center gap-1">
          <span className="text-[10px] font-mono uppercase tracking-[0.3em] text-white/50">Initializing Neural Link</span>
          <span className="text-[10px] font-mono font-bold text-white tracking-widest">{Math.round(progress)}%</span>
        </div>
      </div>
    </Html>
  )
...
function Model({ zoomed, setZoomed }: { zoomed: boolean, setZoomed: (v: boolean) => void }) {
      ...
      {techs.map((tech, i) => (
        <Float key={i} speed={2} rotationIntensity={0.5} floatIntensity={0.5}>
          <Html
            ...
            occlude
          >
            <div className="flex items-center gap-2 px-2.5 py-1 bg-black/40 backdrop-blur-md border border-white/10 rounded-full whitespace-nowrap active:scale-95 transition-all duration-300 group cursor-pointer hover:border-white/40 hover:bg-black/60 shadow-[0_0_15px_rgba(0,0,0,0.3)]">
              <div 
...
function SceneContent() {
  ...
  return (
    <>
      ...
      <pointLight position={[0, 2, -5]} intensity={40} color="#ffffff" /> {/* Back light for depth */}
